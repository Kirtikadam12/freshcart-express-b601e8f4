-- Create role enum
CREATE TYPE public.app_role AS ENUM ('buyer', 'delivery');
...
FOR SELECT
TO authenticated
USING (public.has_role(auth.uid(), 'delivery') AND (delivery_id = auth.uid() OR delivery_id IS NULL));
...
FOR UPDATE
TO authenticated
USING (public.has_role(auth.uid(), 'delivery') AND delivery_id = auth.uid());
